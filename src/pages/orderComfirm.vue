          ...
          <div class="order-content">
            ...
            <section class="address-wrapper">
              ...
              <div class="address">
                ...
                >
                  <h4 class="consignee">{{item.people}}</h4>
            ...
            <section class="product-wrapper">
              ...
              <ul class="product-list">
                <li class="product-item" v-for="(item,index) in selectedProductList" :key="index">
                  ...
                  <div class="col col-price">{{item.price}} x {{item.num}}</div>
                  <div class="col col-total">{{item.price*item.num}}元</div>
                </li>
              </ul>
            </section>
            <!-- 配送方式 -->
      ...
      <template v-slot:dialog-body>
        <form class="new-address">
          ...
          <div class="input-group">
            <div
              class="input-item"
              :class="{'input-focus':target == 1,'input-active':newAddress.people != ''}"
            >
              ...
              <input
                ...
                @focus="target = 1"
                @blur="target = 0"
                v-model="newAddress.people"
              />
            </div>
            <!-- 收货人手机号 -->
            <div
              class="input-item"
              :class="{'input-focus':target == 2,'input-active':newAddress.phone != ''}"
            >
          ...
          <div class="input-group textarea-wrapper">
            <div
              class="input-item"
              :class="{'input-focus':target == 3,'input-active':newAddress.location != ''}"
            >
          ...
          <div class="input-group tag-wrapper">
            <div
              class="input-item"
              :class="{'input-focus':target == 4,'input-active':newAddress.tag != ''}"
            >
...
import {getCartList, getUrl} from "../api";
export default {
  name: "confirm-order",
  data() {
    return {
  ...
  computed: {
    ...
    userInfo() {
      ...
      let { people,phone  } = this.selectedAddress;
      return `${people} ${phone}`;
    },
    // 总金额
    productTotal() {
  ...
  methods: {
    ...
    toOrder() {
      ...
      let numList = [];
      let priceList = [];
      this.selectedProductList.forEach(function(v){
          idList.push(v.productId);
          numList.push(v.num);
          priceList.push(v.price);
      });
      var params = new URLSearchParams();
      params.append("idList",idList);
      params.append("numList",numList);
    ...
    getCart(resolve) {
      getCartList(this.$store.state.username).then(res => {
        console.log(res);
        this.selectedProductList = res.data.data.filter(item => {
          return item.check;
        });
        resolve();
      });
    ...
    getAddress() {
      ...
      this.$axios({
        method: 'post',
        url: getUrl() + `/shippings/${this.$store.state.username}`,
        contentType: 'application/x-www-form-urlencoded',
      }).then(res => {
        console.log(res);
        this.addressList = res.data.data.slice(0, 3);
      });
    },
    // 增加收货地址
    addNewAddress() {
      ...
      params.append("people",this.newAddress.people);
      params.append("phone",this.newAddress.phone);
      params.append("location",this.newAddress.location);
      params.append("tag",this.newAddress.tag);
      this.$axios({
        method: 'post',
    ...
    delAddress() {
      ...
      this.$axios({
        method: 'post',
        url: getUrl() + `/address/delete/${this.shippingId}`,
        contentType: 'application/x-www-form-urlencoded',
      }).then(() => {
        this.getAddress();
