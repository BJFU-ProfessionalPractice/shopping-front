        ...
        <div class="product-list-wrapper">
          ...
          <section>
            ...
            <div class="import-product">
              <a href="javascript:;">
                <img v-lazy="'http://172.23.101.253:8082/public/imgs/phone/mix-alpha.jpg'" alt />
              </a>
            </div>
            <!-- 产品列表 -->
              ...
              <ul v-for="(phone,index) in phoneList" :key="index">
                <li class="product-list-item" v-for="(item,index) in phone" :key="index">
                  <a :href="'/#/product/'+item.productId" target="_blank">
                    <img v-lazy="item.productPicture" alt />
                    <p class="product-name">{{item.productName}}</p>
                    <p class="product-subtitle">{{item.productTitle}}</p>
                    <p class="product-price">{{item.productSellingPrice}}元起</p>
                  </a>
                </li>
              </ul>
            </div>
...
export default {
  ...
  mounted() {
    getProductList({ categoryId: "1", pageSize: 8 }).then(res => {
      let phoneList = [];
      phoneList.push(res.data.data.slice(0, 4), res.data.data.slice(4, 8));
